...
def LinkPrio(linkPrio1, linkPrio2, enablePlot):
    ...
    for (i, (res, inc, outc)) in enumerate(zip([dl1DataOut, dl2DataOut], [dl1DataRate, dl2DataRate], [dl1Rate, dl2Rate])):
        ...
        mpl.rc('lines', linestyle='solid')
        ax2.plot(xs, dres(xs) * outc(res)(xs) / (1024*1024) + i*0.15, color, label='task ' + str(i + 1) + ' DL')
...
#ax1.legend(handles, ['task 1 DL', 'task 1 CPU', 'task 2 DL'], title='Limited by:', loc='upper left')
ax1.tick_params(
    axis='both',
    which='both',
    bottom=True,
...
fig.set_size_inches(w * 1.05, h * 1.05)
fig.tight_layout()
